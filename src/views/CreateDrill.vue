...
const submit = async () => {
  ...
  const newDrill = {
    ...form.value,
    createdBy: store.currentUser?.name || 'Unknown Trainer'
  }

  try {
    await store.createDrill(newDrill)
    router.push('/drills')
  } catch (error) {
    console.error('Failed to create drill:', error)
    alert('Failed to create drill. Please check your inputs and try again.')
  }
}
const limitMaxScore = () => {
  ...
  <div class="max-w-4xl mx-auto">
    ...
    >
      ...
      <div class="pt-6 border-t border-gray-200">
        ...
        <div v-if="form.questions.length === 0" class="text-center py-10 bg-slate-50 rounded-2xl border-2 border-dashed border-gray-200">
          <p class="text-gray-400">No questions added yet. Click the buttons above to add questions.</p>
        </div>
